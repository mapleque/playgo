...
export default class DataBus {
    ...
    this.conn.onMessage = message => {
      ...
      switch (message.type) {
        case 'game_list': this.updateGameList(message.data)
          break
        case 'game': this.updateGame(message.data, message.token)
          break
      }
    }
  }
  ...
  updateGame (game, token) {
    this.timer = new Date()
    this.startTime = new Date()
    this.wh = token == game.player2
    this.waiting = this.wh != game.turn
    this.trying = !this.waiting
    this.showChesses = game.alive_chesses
  }
  updateGameList (data) {
