...
func RunWsGateway(serveAddr, serverAddr string) {
	...
	http.Handle("/", websocket.Handler(func(ws *websocket.Conn) {
		go serveSend(ws, stream)
		serveReceive(ws, stream)
		log.Printf("websocket disconnect")
	}))
...
func serveSend(ws *websocket.Conn, stream PlaygoService_OperationClient) {
	for {
...
func serveReceive(ws *websocket.Conn, stream PlaygoService_OperationClient) {
	for {
		var message string
